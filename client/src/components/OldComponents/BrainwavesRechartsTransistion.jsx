...
function animateWaveSmooth(canvas, oldData, newData, color, durationMs = 800) {
    if (!canvas || !oldData || !newData) return () => { };
    ...
    function frame(ts) {
        ...
        for (let i = 0; i <= N; i++) {
            const interpolated = lerp(oldData[i], newData[i], t);
            const x = (i / N) * w;
            const y = mid - interpolated * scale;
            i === 0 ? ctx.moveTo(x, y) : ctx.lineTo(x, y);
        }
...
const WaveRow = forwardRef(({ band, isLast }, ref) => {
  ...
  const cancelRef = useRef(null);
    ...
    const refresh = useCallback(() => {
        ...
        const newData = generateEEG(band);

        cancelRef.current = animateWaveSmooth(
            canvasRef.current,
            dataRef.current,
            newData,
            band.color,
  ...
  // Initial draw
    useEffect(() => {
        ...
        drawWave(canvasRef.current, dataRef.current, band.color);
